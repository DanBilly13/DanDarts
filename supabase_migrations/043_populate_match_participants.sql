-- =====================================================
-- DanDarts Database Migration 043
...
-- 
-- SAFETY FEATURES:
-- 1. Validates row counts
-- 2. Checks for NULL values
-- 3. Rollback script included
-- =====================================================
...
-- The players column is stored as double-encoded JSON (TEXT with escaped quotes)
-- We need to: TEXT -> JSON (removes outer quotes) -> JSONB (parse array)
INSERT INTO public.match_participants (match_id, user_id, is_guest, display_name)
SELECT 
    m.id as match_id,
    (player->>'id')::UUID as user_id,
    COALESCE((player->>'isGuest')::BOOLEAN, false) as is_guest,
    player->>'displayName' as display_name
FROM 
    public.matches m,
    jsonb_array_elements((m.players::text)::json::jsonb) as player
